  ...
  const verifyEmail = async (token) => {
    try {
      const response = await axios.post(
        `https://paircular-server-vdwt.onrender.com/api/v1/auth/verify-email/${verificationToken}`
      );
    ...
    <main>
      <div className="flex items-center justify-center min-h-screen">
        <div className="bg-wizard w-full px-8 py-6 rounded shadow-md max-w-md">
          ...
          <h1 className="text-sm text-white font-bold mb-4">
            <center>Verifying Email...</center>
          </h1>
          {loading && (
          ...
          {!loading && (
            <div className="flex items-center justify-center">Verified</div>
          )}
        </div>
      </div>
      <ToastContainer />
